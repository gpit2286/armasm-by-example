...
@           Space needs to be at least 12 bytes 
@           Also appends \0 to end of number 
@   Output: 
...
itoa: 
    push    {r4-r10}        @ save registers we will use 
	...
	and		r10, r5, #0x80000000 @ mask first bit 
	cmp		r10, #0			
	beq		findstart		@ if r10 is still zero, then it's not negative 
	@ change negative to positive 
	sub		r5, #1			@ subtract one 
	mvn		r5, r5			@ flip bits 	
...
findstart:
    ...
    ble     negprint       @ if less than number, go to printing 
    sub     r7, #1          @ if still bigger than num to print, 
                            @ decrement pow and try again 
    b       findstart   
    @ process number and print 
negprint:
	cmp		r10, #0			@ check to see if neg sign needs to be printed 
	beq		finddigit		@ if zero, it was originally positive
	mov		r0, #'-'		@ store neg sign ascii 
	strb	r0, [r4], #1	@ store neg sign and increment  
...
write:
    ...
    b       finddigit 
exit: 
    mov     r8, #0          @ store null bit to end 
    strb    r8, [r4]
